...
var microframes = 0;
var smartBots = false;
var fastUpdate = false;
var autoDispurse = false;
var turboMode = false;
var supaTurbo = false;
...
function getBotSteering(idx) {
 const bot = cars[idx];
 let nearestDist = Infinity;
 let nearestCar = null;
 if (keys.includes(" ")||dispursetimer>0) {
  if (idx === 0) {
   nearestCar = {
    num: 1,
    color: "#FF0000",
 ...
 const dx = nearestCar.pos.x - bot.pos.x;
 const dy = nearestCar.pos.y - bot.pos.y;
 let targetAngle = Math.atan2(dy, dx) * (180 / Math.PI);
 targetAngle = trueMod(targetAngle, 360);
 let angleDiff = targetAngle - bot.rotation;
 angleDiff = trueMod(angleDiff + 180, 360) - 180;
 return angleDiff;
}
...
function botupdate() {
 ...
 if (botsActive) {
  if (smartBots) {
   if (dispursetimer>0) dispursetimer--;
   for (let c=1;c<cars.length;c++) {
    if (Math.round(Math.random()*39)==0&&autoDispurse) dispursetimer=20;
    car = cars[c]
    let diff = getBotSteering(c);
    if (Math.abs(diff) <= 15) {
...
function control() {
 botsActive = document.getElementById("bots").checked;
 smartBots = document.getElementById("smart").checked;
 fastUpdate = document.getElementById("fast").checked;
 autoDispurse = document.getElementById("disp").checked;
 if (smartBots) {
  microframes = (microframes+1) % 5;
 } else {
  microframes = (microframes+1) % 20;
 }
 if (microframes===0||fastUpdate) {botupdate();}
...
function loop() {
 if (turboMode) {
  for (let x=0;x<10;x++) {control();}
 } else if (supaTurbo) {
  for (let x=0;x<100;x++) {control();}
 } else {
  control();
 }
 draw();
...
document.getElementById("turbo").addEventListener("change", function (e) {
 if (document.getElementById("turbo").checked) {
  supaTurbo = false;
  document.getElementById("turbo2").checked = false;
  turboMode = true;
 } else {
  turboMode = false;
 }
...
document.getElementById("turbo2").addEventListener("change", function (e) {
 if (document.getElementById("turbo2").checked) {
  turboMode = false;
